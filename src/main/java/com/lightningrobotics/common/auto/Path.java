...
public class Path {
    ...
    public Path(String name, String jsonPath, boolean reversed) {
        ...
        try {
            java.nio.file.Path trajectoryFile = Filesystem.getDeployDirectory().toPath().resolve(this.jsonPath);
            pathTrajectory = TrajectoryUtil.fromPathweaverJson(trajectoryFile);
        } catch (IOException ex) {
            DriverStation.reportError("Unable to open trajectory: " + this.jsonPath, ex.getStackTrace());
        }   
